...
$products = array_values(array_filter($allProducts, function ($p): bool {
  ...
  return strpos($category, 'pastry') !== false;
}));
?>
      ...
      <?php else: ?>
        <?php foreach ($products as $p): ?>
          <?php
            $id = (int) ($p['id'] ?? 0);
            $name = (string) ($p['name'] ?? '');
            $category = strtoupper((string) ($p['category'] ?? 'Pastry'));
            $price = (float) ($p['price'] ?? 0);
            $image = (string) ($p['image'] ?? '../Assets/Carousel.png');
            $description = 'A premium ' . $name . ' made to complement your coffee menu. Fresh taste and reliable quality for daily selling.';
          ?>
